...
class RoiToPoint:
    ...
    """
    Tracks exit in image
    """

    def __init__(self):
        rospy.init_node('exit_tracking')
        self.sub_roi = rospy.Subscriber('exit/roi', CameraInfo, self.roi_callback)
        self.pub_point = rospy.Publisher("exit/rawpoint", PointStamped, queue_size=1)

        self.camera_proj = None
        self.object_width = rospy.get_param('object_width', 0.2)
    ...
    def roi_callback(self, msg: CameraInfo):
        ...
        try:
            trans = self.tfBuffer.lookup_transform('world', 'camera_link', msg.header.stamp, rospy.Duration(0.1)).transform
